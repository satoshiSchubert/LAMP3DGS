	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// pair).
			/*
				pix = rgb[0] * alpha[0] + (1-alpha[0])*( rgb[1] * alpha[1] + (1-alpha[1])*(...) )
						+ bg_color * PI(1-alpha[0~i])
				pixa = alpha[0] + (1-alpha[0])*( alpha[1] + (...) )
				The formulation of Alpha is the same as pix, with factor equals to 1.0
				dL/drgb = dL/dpix * dpix/drgb
					T(loop start) = PI(1-alpha[0~i])
					T(current) = T(loop start)/(1.f-alpha)
					dpix/drgb = PI(1-alpha[0~i-1])*alpha[i] = T(current)*alpha
				dL/dalpha = dL/dpix * dpix/dalpha + dL/dpixa(input) * dpixa/dalpha(same as dL/drgb)
					dpix/dalpha = T(current)*(rgb[i] - (...accum_rec)) - bg_color*PI(1-alpha[0~i-1])*PI(1-alpha[i+1~n])
					            = ... - bg_color*T_final/(1-alpha[i])
				** dL/dmean2d = dL/dalpha * dalpha/dmean2d
					mean2d don't affect normal at pixel
			*/
