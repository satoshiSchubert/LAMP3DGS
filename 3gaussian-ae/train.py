...
def readAllConfig(kargs : Param):
    ...
    def getViewMatrix(pos, lookat):
        up = np.array([[0, 0.0, 1]], dtype=np.float32).repeat(pos.shape[0], axis=0)
        front = lookat - pos
...
def readEnvDataset(kargs):
    ...
    rawBase = './furball/{}/{}.exr'
    denoisedBase = './furball/{}/{}-denoised.exr'
    bgBase = './furball/{}/{}.exr'
...
class Scene:
    ...
    def startTraining(self, kargs, vis_it=20000):
        ...
        for iter in range(firstIter, maxIter + 1):
            ...
            if iter % 1000 == 0:
                self.gaussian.oneupSHdegree() # 增加SH维度
            ...
            if kargs.pipeline == 'legacy':
                ...
                # imgOutput: MLP出的结果，geoOutput: 3DGS直出结果
