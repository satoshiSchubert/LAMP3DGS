...
def sample_gaussians(world_pos, l, n):
    ...
    while len(gaussians) < n:
        ...
        xyz = (pos1 + pos2) / 2
        length = 0.1*np.linalg.norm(pos2 - pos1) # scale一下，不然挤成一坨
        scs = np.array([0.2 * length, 0.2 * length, length], dtype=np.float32)  # 3-channel scale
        scs = np.log(scs)
...
def exportExr(case_pth, target_path = r"E:\KeProduct\LAMP3DGS\3gaussian-ae\furball/gt-case/"):
    ...
    if case_pth.split(".")[-1] == "gz":
        with gzip.open(case_pth, 'rb') as lf:
            localData = pickle.load(lf)
    elif case_pth.split(".")[-1] == "zst":
        with open(case_pth, 'rb') as lf:
            dctx = zstd.ZstdDecompressor()
            localData = pickle.loads(dctx.decompress(lf.read()))
    else:
        assert False, "Invalid file format."
    radiance = localData['radiance'].reshape(-1, 512, 512, 3)
    for i, frame in enumerate(radiance):
        pyexr.write(target_path + f"{i}.exr", frame)


def main_sampling(case_pth):
    localData = None
    if case_pth.split(".")[-1] == "gz":
        with gzip.open(case_pth, 'rb') as lf:
            localData = pickle.load(lf)
    ...
    # 调换y和z匹配3dgs坐标系
    #world_pos = world_pos[..., [0,2,1]]
    print(f"Resized world_pos shape: {world_pos.shape}")

    # L和N与resize后的分辨率有关！因为实质是利用下采样导致的稀疏化来采样Gaussian
    l = 2 # Distance threshold for two sampled points
    n = 2 # Maximum number of Gaussians to sample per frame, so total num is n*frame_num
    all_gaussians = []
